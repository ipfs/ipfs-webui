...
const Pin = (t, pin, status = Status.Pending, onDismiss) => {
  ...
  return (
    <li className="flex w-100 bb b--light-gray items-center f6 charcoal" key={cid}>
      <GlyphPinCloud
...
const PinsStatuses = ({
  ...
  return (
    <div>
      <h3>
        {pendingPins.length + failedPins.length + completedPins.length > 0
          ? t('remotePinningInProgress')
          : t('noPinsInProgress')}
      </h3>
