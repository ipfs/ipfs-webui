      ...
      { !links ? null : (
        <div className='bg-near-white mt2 overflow-scroll'>
          <table className='lh-copy tl f6 w-100 dt--fixed' cellSpacing='0'>
            <thead>
              <tr>
                <th className='fw4 bb b--black-20 pv2 ph2' style={{width: '40px'}} />
            ...
            <tbody className='fw4'>
              {links.slice(0, 100).map(({name, size, multihash}, i) => (
                <tr className='pointer striped--light-gray' key={`${multihash}-${name}`}>
                  <td className='pv1 ph2 silver truncate monospace tr f7'>{i}</td>
                  <td className='pv1 ph2 dark-gray truncate teal'>{name}</td>
                  <td className='pv1 pr4 mid-gray truncate monospace tr f7'>{humansize(size)}</td>
                  <td className='pv1 pr2 mid-gray monospace f7'>{multihash}</td>
                </tr>
              ))}
            </tbody>
          </table>
