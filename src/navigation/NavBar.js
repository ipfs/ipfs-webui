...
const NavLink = ({ to, icon, alternative, disabled, children }) => {
  const Svg = icon;
  ...
  return (
    ...
    <a href={disabled ? null : href} className={anchorClass} role="menuitem" title={children}>
      <div className="db ph2 pv1">
        ...
        <div
          className={`${active ? 'o-100' : 'o-50'} db f6 tc montserrat ttu fw1 `}
...
export const NavBar = ({ t }) => {
  ...
  return (
    <div
      className="h-100 fixed-l flex flex-column justify-between"
    ...
    >
      <div className="flex flex-column">
        ...
        <div className="db overflow-x-scroll overflow-x-hidden-l nowrap tc" role="menubar">
          <NavLink to="/" alternative="status" icon={StrokeMarketing}>
            {t('status:title')}
          </NavLink>
          <NavLink to="/files" icon={StrokeWeb}>
            {t('files:title')}
          </NavLink>
          <NavLink to="/explore" icon={StrokeIpld}>
            {t('explore:tabName')}
          </NavLink>
          <NavLink to="/peers" icon={StrokeCube}>
            {t('peers:title')}
          </NavLink>
          <NavLink to="/settings" icon={StrokeSettings}>
            {t('settings:title')}
          </NavLink>
        </div>
      </div>
      <div className="dn db-l navbar-footer mb2 tc center f7 o-80 glow">
        {gitRevision && (
          <div className="mb1">
            <a className="link white" href={revisionUrl} target="_blank" rel="noopener noreferrer">
              {t('app:terms.revision')} {gitRevision}
            </a>
          </div>
        )}
        <div className="mb1">
          <a className="link white" href={codeUrl} target="_blank" rel="noopener noreferrer">
            {t('app:nav.codeLink')}
          </a>
        </div>
        <div>
          <a className="link white" href={bugsUrl} target="_blank" rel="noopener noreferrer">
            {t('app:nav.bugsLink')}
          </a>
        </div>
      </div>
...
export default connect(withTranslation()(NavBar));
