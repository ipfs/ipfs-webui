...
const locationCellRenderer =
  ...
  ({ rowData }) => {
    ...
    const location = rowData.isPrivate ? (
      ...
      rowData.isNearby ? (
        <span>
          {rowData.location} <span className="charcoal-muted">({t('nearby')})</span>
        </span>
      ) : (
        rowData.location
      )
...
const connectionCellRenderer =
  t =>
  ({ rowData }) => {
    const ref = React.createRef();
    const { address, direction, peerId } = rowData;
    const p2pMultiaddr = `${address}/p2p/${peerId}`;
    const title = direction != null ? `${address}\n(${renderDirection(direction, t)})` : address;
...
export const PeersTable = ({ className, t, peerLocationsForSwarm, selectedPeers }) => {
  ...
  const filteredPeerList = useMemo(() => {
    ...
    return awaitedPeerLocationsForSwarm.filter(
      ({ location, latency, peerId, connection, protocols }) => {
        if (location != null && location.toLowerCase().includes(filterLower)) {
          return true;
        }
        if (
          latency != null &&
          [latency, `${latency}ms`].some(str => str.toString().includes(filterLower))
        ) {
          return true;
        }
            ...
            <>
              <Table
                ...
                rowClassName={rowInfo =>
                  rowClassRenderer(rowInfo, awaitedPeerLocationsForSwarm, selectedPeers)
                }
                width={width}
                height={tableHeight}
                headerHeight={32}
              ...
              >
                <Column
                  ...
                  cellRenderer={locationCellRenderer(t)}
                  dataKey="location"
                  width={450}
                ...
                <Column
                  ...
                  cellRenderer={latencyCellRenderer}
                  dataKey="latency"
                  width={200}
                ...
                <Column
                  ...
                  cellRenderer={peerIdCellRenderer(t)}
                  dataKey="peerId"
                  width={250}
                ...
                <Column
                  ...
                  cellRenderer={connectionCellRenderer(t)}
                  dataKey="connection"
                  width={250}
                  className="f6 charcoal truncate pl2"
                />
                <Column
                  label={t('protocols')}
                  cellRenderer={protocolsCellRenderer(t)}
                  dataKey="protocols"
                  width={520}
