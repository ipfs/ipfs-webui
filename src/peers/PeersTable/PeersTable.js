...
export class PeersTable extends React.Component {
    ...
    locationCellRenderer = ({ rowData }) => {
        ...
        return (<CopyToClipboard text={value} onCopy={() => copyFeedback(ref, this.props.t)}>
    ...
    latencyCellRenderer = ({ cellData, rowData }) => {
        ...
        if (cellData == null)
            return (<span className='dib o-40 no-select' style={style}>-</span>);
        return (<span className='dib no-select'>{latency}</span>);
    };
    peerIdCellRenderer = ({ cellData: peerId }) => {
        const ref = React.createRef();
        const p2pMultiaddr = `/p2p/${peerId}`;
        return (<CopyToClipboard text={p2pMultiaddr} onCopy={() => copyFeedback(ref, this.props.t)}>
    ...
    protocolsCellRenderer = ({ rowData, cellData }) => {
        ...
        return (<CopyToClipboard text={protocols} onCopy={() => copyFeedback(ref, this.props.t)}>
    ...
    render() {
        ...
        return (<div className={`bg-white-70 center ${className}`} style={{ height: `${tableHeight}px`, maxWidth: 1764 }}>
        {peerLocationsForSwarm && <AutoSizer disableHeight>
          {({ width }) => (<Table className='tl fw4 w-100 f6' headerClassName='teal fw2 ttu tracked ph2 no-select' rowClassName={(rowInfo) => this.rowClassRenderer(rowInfo, peerLocationsForSwarm)} width={width} height={tableHeight} headerHeight={32} rowHeight={36} rowCount={peerLocationsForSwarm.length} rowGetter={({ index }) => sortedList[index]} sort={this.sort} sortBy={sortBy} sortDirection={sortDirection}>
...
export default connect('selectPeerLocationsForSwarm', 'selectSelectedPeers', withTranslation('peers')(PeersTable));
