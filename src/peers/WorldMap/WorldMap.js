...
const WorldMap = ({ t, className, selectedPeers, doSetSelectedPeers }) => {
  ...
  const handleMapPinMouseEnter = useCallback(
    (peerIds, element) => {
      if (!element) return;
  ...
  const handleMapPinMouseLeave = useCallback(() => {
    setSelectedTimeout(setTimeout(() => doSetSelectedPeers({}), 600));
  }, [doSetSelectedPeers]);
    ...
    <div className="flex flex-column">
      <div className={`relative ${className}`}>
        <div className="mb1 flex flex-column items-center">
          <div
            className="relative no-events"
            style={{
              width,
              height,
              background: `transparent url(${staticMapSrc}) center no-repeat`,
            ...
            <GeoPath width={width} height={height}>
              {({ path }) => (
                <MapPins
                  ...
                  height={height}
                  path={path}
                  handleMouseEnter={handleMapPinMouseEnter}
                  handleMouseLeave={handleMapPinMouseLeave}
                />
              )}
            </GeoPath>
            {selectedPeers?.peerIds && (
              <Popover
                show={!!(selectedPeers.top && selectedPeers.left)}
                top={selectedPeers.top}
                left={selectedPeers.left}
                align="bottom"
                handleMouseEnter={handlePopoverMouseEnter}
                handleMouseLeave={handleMapPinMouseLeave}
              >
      ...
      <div className="relative flex justify-end pt2 pb4">
        <div className="f6 p2 no-select flex items-center">
          ...
          <i
            ...
            style={{
              width: getDotsSize(1) * 2,
              height: getDotsSize(1) * 2,
              backgroundColor: getDotsColor(1),
            }}
          ></i>
          1-10
          <i
            ...
            style={{
              width: getDotsSize(50) * 2,
              height: getDotsSize(50) * 2,
              backgroundColor: getDotsColor(50),
            }}
          ></i>{' '}
          10-100
          <i
            className="mapDotExplanation ml3 mr1"
            style={{
              width: getDotsSize(110) * 2,
              height: getDotsSize(110) * 2,
              backgroundColor: getDotsColor(110),
            }}
          ></i>
          100+
        </div>
...
const MapPins = connect(
  'selectPeersCoordinates',
  ({ width, height, path, peersCoordinates, handleMouseEnter, handleMouseLeave }) => {
      ...
      {peers
        ...
        .map((peer, index) => {
          if (index === MAX_PEERS && peers.length > MAX_PEERS) {
            return (
              <div className="f7 pa1 self-end" key="worldmap-more-label">
                {t('plusPeers', { number: peers.length - MAX_PEERS })}
              </div>
            );
          }
