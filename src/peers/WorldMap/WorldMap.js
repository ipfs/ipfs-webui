        ...
        <div className='mb1 flex flex-column items-center'>
          <div className="relative no-events" style={{ width, height, background: `transparent url(${staticMapSrc}) center no-repeat`, backgroundSize: 'auto 100%' }}>
            <GeoPath width={width} height={height}>
              {({ path }) => (<MapPins width={width} height={height} path={path} handleMouseEnter={handleMapPinMouseEnter} handleMouseLeave={handleMapPinMouseLeave}/>)}
            </GeoPath>
            {selectedPeers?.peerIds && (<Popover show={!!(selectedPeers.top && selectedPeers.left)} top={selectedPeers.top} left={selectedPeers.left} align='bottom' handleMouseEnter={handlePopoverMouseEnter} handleMouseLeave={handleMapPinMouseLeave}>
...
const getDotsColor = (numberOfDots) => {
    if (numberOfDots < 10)
        return 'rgba(150, 204, 255, 0.6)';
    if (numberOfDots < 100)
        return 'rgba(53, 126, 221, 0.6)';
...
const PeerInfo = connect('selectPeerLocationsForSwarm', ({ ids, peerLocationsForSwarm: allPeers, t }) => {
    ...
    return (<div className="f6 flex flex-column justify-center">
      {peers.sort((a, b) => a.address.localeCompare(b.address)).map((peer, index) => {
            ...
            if (index > MAX_PEERS)
                return null;
            return (<div className="pa2 f7 flex items-center monospace" key={peer.peerId}>
...
export default connect('selectSelectedPeers', 'doSetSelectedPeers', withTranslation('peers')(WorldMap));
