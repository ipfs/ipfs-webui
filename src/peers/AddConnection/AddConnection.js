...
class AddConnection extends React.Component {
    ...
    get description() {
        ...
        return (<div className='mb3 flex flex-column items-center'>
        ...
        <Overlay show={open} onLeave={this.toggleModal}>
          <Modal onCancel={this.toggleModal}>
            <ModalBody title={t('addConnection')} Icon={Icon}>
              ...
              <input name='maddr' type='text' value={this.state.maddr} onChange={this.onChange} onKeyPress={this.onKeyPress} required className={`input-reset charcoal ba b--black-20 br1 pa2 mv2 db w-90 center focus-outline ${this.inputClass}`}/>

              <Checkbox className='dib' onChange={this.onPeeringToggle} checked={this.state.permanent} label={<span className='f6 charcoal-muted lh-copy'>{t('addPermanentPeer')}</span>}/>
            </ModalBody>
            ...
            {loading && (<div className="flex items-center justify-center absolute top-0 left-0 right-0 bottom-0">
