...
const File = ({
  ...
  const checkIfDir = monitor => {
    ...
    if (item.name) {
      return type === 'directory' && name !== item.name && !selected;
    }
    ...
    <div ref={drop}>
      <div className={className} style={styles} onContextMenu={handleCtxRightClick} ref={drag}>
        ...
        <button
          ...
          aria-label={name === '..' ? t('previousFolder') : t('fileLabel', { name, type, size })}
        >
        ...
        <div className="ph2 pv1 flex-none hide-child dn db-l tr mw3 w-20 transition-all">
          <button
            ...
            onClick={isFailedPin ? onDismissFailedPin : () => onSetPinning([{ cid, pinned }])}
          >
            <PinIcon
        ...
        <button
          ref={dotsWrapper}
          className="ph2 db button-inside-focus file-context-menu"
          style={{ width: '2.5rem' }}
