...
const BarOption = ({ children, text, isLink = false, className = '', ...etc }) => (
  <div className={classNames(className, 'tc pa3', etc.onClick && 'pointer')} {...etc}>
    ...
    <span className={`nowrap db ttu f6 montserrat fw4 ${isLink ? 'link' : 'charcoal-muted'}`}>
      {text}
    </span>
  </div>
);
    ...
    return (
      <div className="db flex-l justify-between items-center">
        <div className="mb3 overflow-hidden mr2">
          <Breadcrumbs
            className="joyride-files-breadcrumbs"
        ...
        <div className="mb3 flex justify-between items-center bg-snow-muted joyride-files-add">
          {pinsInQueue > 0 && (
            <a href="#/pins" alt={t('pinningQueue')} title={t('pinningQueue')} className="ml3">
              <GlyphPinCloud style={{ width: '3rem' }} className="fill-teal PendingAnimation" />
            </a>
          )}

          <BarOption
            title={t('currentLocationDescription')}
            text={hasUpperDirectory ? t('currentLocation') : t('currentLocationRoot')}
          >
            {hasUpperDirectory ? (
              <span>
                {size(currentDirectorySize)}
                <span className="f5 gray">/{size(filesSize)}</span>
              </span>
            ) : (
              size(filesSize)
            )}
