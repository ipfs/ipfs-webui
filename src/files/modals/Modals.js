...
export { NEW_FOLDER, SHARE, RENAME, DELETE, ADD_BY_PATH, CLI_TUTOR_MODE, PINNING, PUBLISH }
  ...
  render () {
    ...
    return (<div>
        <Overlay show={show === NEW_FOLDER && readyToShow} onLeave={this.leave}>
          <NewFolderModal className='outline-0' onCancel={this.leave} onSubmit={this.makeDir}/>
        </Overlay>

        <Overlay show={show === SHARE && readyToShow} onLeave={this.leave}>
          <ShareModal className='outline-0' link={link} onLeave={this.leave}/>
        </Overlay>

        <Overlay show={show === RENAME && readyToShow} onLeave={this.leave}>
          <RenameModal className='outline-0' {...rename} onCancel={this.leave} onSubmit={this.rename}/>
        </Overlay>

        <Overlay show={show === DELETE && readyToShow} onLeave={this.leave}>
          <RemoveModal className='outline-0' {...this.state.delete} onCancel={this.leave} onRemove={this.delete}/>
        </Overlay>

        <Overlay show={show === ADD_BY_PATH && readyToShow} onLeave={this.leave}>
          <AddByPathModal className='outline-0' onSubmit={this.onAddByPath} onCancel={this.leave}/>
        </Overlay>
        ...
        <Overlay show={show === PINNING && readyToShow} onLeave={this.leave}>
          <PinningModal file={this.state.pinning.file} className='outline-0' onCancel={this.leave} onPinningSet={this.onPinningSet}/>
        </Overlay>

        <Overlay show={show === PUBLISH && readyToShow} onLeave={this.leave}>
          <PublishModal file={this.state.publish.file} className='outline-0' onLeave={this.leave} onSubmit={this.publish}/>
        </Overlay>
      </div>)
  }
}
