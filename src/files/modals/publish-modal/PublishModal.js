...
export const PublishModal = ({ t, tReady, onLeave, onSubmit, file, ipnsKeys, publicGateway, className, doFetchIpnsKeys, doUpdateExpectedPublishTime, expectedPublishTime, ...props }) => {
  ...
  const modalBody = () => {
    if (link) {
      return (<div>
          ...
          <div className='flex center mb2 relative'>
            <input value={link} readOnly className={'input-reset flex-grow-1 charcoal-muted ba b--black-20 br1 pa2 focus-outline'} style={{ color: `rgba(128, 133, 145, ${copied ? 0.6 : 1})` }} type='text'/>
    ...
    return (<div>
        ...
        <div className="publishModalKeys">
          {ipnsKeys.map((key) => (<button className="flex items-center pa1 hoverable-button" key={key.name} onClick={() => changeKey(key)}>
  ...
  return (<Modal {...props} className={className} onCancel={onLeave}>
...
export default connect('selectIpnsKeys', 'selectExpectedPublishTime', 'selectPublicGateway', 'doFetchIpnsKeys', 'doUpdateExpectedPublishTime', withTranslation('files')(PublishModal))
