...
function RenameModal ({ t, tReady, onCancel, onSubmit, filename, folder, className, ...props }) {
  ...
  return (<TextInputModal onCancel={onCancel} onSubmit={onSubmit} mustBeDifferent className={className} defaultValue={filename} title={t(`renameModal.title${context}`)} description={t(`renameModal.description${context}`)} Icon={PencilIcon} submitText={t('app:actions.save')} {...props}/>)
}
