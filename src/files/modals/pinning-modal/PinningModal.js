    ...
    return (<Modal {...props} className={className} onCancel={onCancel}>
      <ModalBody title={t('pinningModal.title')} Icon={Icon}>
        <div className="pinningModalContainer">
          ...
          {pinningServices.map(({ icon, name }, index) => (<button className="flex items-center pa1 hoverable-button" key={name} onClick={() => selectService(name)}>
...
export default connect('selectPinningServices', 'selectRemotePins', 'selectNotRemotePins', 'doSelectRemotePinsForFile', 'doGetFileSizeThroughCid', 'doFetchPinningServices', 'doFetchRemotePins', withTranslation('files')(PinningModal));
