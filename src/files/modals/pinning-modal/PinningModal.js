...
export const PinningModal = ({
  ...
  const selectedRemoteServices = useMemo(
    () => doSelectRemotePinsForFile(file, remotePins, notRemotePins, pendingPins),
    [doSelectRemotePinsForFile, file, remotePins, notRemotePins, pendingPins]
  );
  const [selectedServices, setSelectedServices] = useState([
    ...selectedRemoteServices,
    ...[file.pinned && 'local'],
  ]);
    ...
    <Modal {...props} className={className} onCancel={onCancel}>
      <ModalBody title={t('pinningModal.title')} Icon={Icon}>
        <div className="pinningModalContainer">
          <button
            className="flex items-center pa1 hoverable-button"
            key={t('pinningModal.localNode')}
            onClick={() => selectService('local')}
          >
            <Checkbox
              className="pv3 pl3 pr1 flex-none"
              checked={selectedServices.includes('local')}
            ...
            <GlyphPin
              ...
              width={32}
              height={32}
              className="mr1 aqua flex-shrink-0"
            />
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          href={complianceReportsHomepage}
        >
      ...
      <ModalActions>
        ...
        <Button
          className="ma2 tc"
          bg="bg-teal"
          onClick={() => onPinningSet(file, selectedServices, file.pinned, selectedRemoteServices)}
        >
          {t('app:actions.apply')}
        </Button>
      </ModalActions>
