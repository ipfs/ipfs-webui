  ...
  [...groupByPath(job?.message?.entries || new Map()).values()].map(item => (
    <li className="flex w-100 bb b--light-gray items-center f6 charcoal" key={item.path}>
      ...
      <span className="gray mh2">
        ...
        |
        {item.entries && (
          <span> {t('filesImportStatus.count', { count: item.entries.length })} | </span>
        )}
...
const LoadingIndicator = ({ complete }) =>
  complete ? (
    <GlyphTick className="green w2 ph1" fill="currentColor" />
  ) : (
    <span className="w2 ph1" />
  );
...
export const FileImportStatus = ({
  ...
  const sortedFilesFinished = useMemo(
    () => filesFinished.sort((fileA, fileB) => fileB.start - fileA.start),
    [filesFinished]
  );
  const [expanded, setExpanded] = useState(initialExpanded);

  const handleImportStatusClose = useCallback(
    ev => {
      doFilesClear();
  ...
  const numberOfImportedItems = !filesFinished.length
    ? 0
    : filesFinished.reduce((prev, finishedFile) => prev + finishedFile.message.entries.length, 0);
  const numberOfPendingItems = filesPending.reduce(
    (total, pending) => total + groupByPath(pending.message.entries).size,
    0
  );
  const progress = Math.floor(
    filesPending.reduce((total, { message: { progress } }) => total + progress, 0) /
      filesPending.length
  );

  return (
    <div
      className="fileImportStatus fixed bottom-1 w-100 flex justify-center"
      style={{ zIndex: 14, pointerEvents: 'none' }}
    >
      <div
        className="relative br1 dark-gray w-40 center ba b--light-gray bg-white"
      ...
      >
        ...
        >
          ...
          <div className="flex items-center">
            <button
              className="fileImportStatusArrow ph0 flex"
              onClick={() => setExpanded(!expanded)}
              aria-expanded={expanded}
            ...
            <button
              className="fileImportStatusCancel ph0 flex"
