...
const Import = (job, t) => [...groupByPath(job?.message?.entries || new Map()).values()].map(item => (<li className="flex w-100 bb b--light-gray items-center f6 charcoal" key={item.path}>
      {viewIcon(item)}
...
const LoadingIndicator = ({ complete }) => complete ? <GlyphTick className="green w2 ph1" fill="currentColor"/> : <span className="w2 ph1"/>
export const FileImportStatus = ({ filesFinished, filesPending, filesErrors, doFilesClear, initialExpanded, t }) => {
  ...
  return (<div className='fileImportStatus fixed bottom-1 w-100 flex justify-center' style={{ zIndex: 14, pointerEvents: 'none' }}>
...
export default connect('selectFilesFinished', 'selectFilesPending', 'selectFilesErrors', 'doFilesClear', withTranslation('files')(FileImportStatus))
