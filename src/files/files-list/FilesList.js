...
export const FilesList = ({
  ...
  const move = (src, dst) => {
    if (selectedFiles.length > 0) {
      ...
      const toMove = selectedFiles.map(({ name, path }) => [path, join(basepath, name)]);
  ...
  const emptyRowsRenderer = () => (
    <Trans i18nKey="filesList.noFiles" t={t}>
      <div className="pv3 b--light-gray bt tc gray f6">There are no available files. Add some!</div>
    </Trans>
  );
  ...
  return (
    <section
      ...
      className={classnames(
        'FilesList no-select sans-serif border-box w-100 flex flex-column',
        className
      )}
    >
      {showLoadingAnimation ? (
          ...
          >
            ...
            <div className="ph2 f6 flex-auto">
              <button
                ...
                onClick={changeSort(sorts.BY_NAME)}
              >
            ...
            <div className="pl2 pr1 tr f6 flex-none dn db-l mw4">
              {pinningServices && pinningServices.length ? (
                <button
                  aria-label={t('app:terms.pinStatus')}
                  onClick={() => doFetchRemotePins(files, refreshPinCache)}
                >
            ...
            <div className="pl2 pr4 tr f6 flex-none dn db-l mw4 w-10">
              <button
                aria-label={t('sortBy', { name: t('size') })}
                onClick={changeSort(sorts.BY_SIZE)}
              >
