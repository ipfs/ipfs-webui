...
export const FilesList = ({ className, files, pins, pinningServices, remotePins, filesSorting, updateSorting, downloadProgress, filesIsFetching, filesPathInfo, showLoadingAnimation, onShare, onSetPinning, onInspect, onDownload, onRemove, onRename, onNavigate, onRemotePinClick, onAddFiles, onMove, doFetchRemotePins, handleContextMenuClick, t }) => {
  ...
  const selectedFiles = useMemo(() => selected
  ...
  }, [files, focused, filesIsFetching, setSelected, setFocused, listRef, onRename, onRemove, selectedFiles, selected, toggleOne, onNavigate, filesRefs, firstVisibleRow])
  useEffect(() => {
  ...
  const emptyRowsRenderer = () => (<Trans i18nKey='filesList.noFiles' t={t}>
  ...
  const rowRenderer = ({ index, key, style }) => {
    ...
    return (<div key={key} style={style} ref={r => { filesRefs.current[allFiles[index].name] = r }}>
        <File {...listItem} pinned={pinsString.includes(listItem.cid.toString())} isMfs={filesPathInfo.isMfs} name={listItem.name} onSelect={toggleOne} onNavigate={onNavigateHandler} onAddFiles={onAddFiles} onSetPinning={onSetPinning} onMove={move} focused={focused === listItem.name} selected={selected.indexOf(listItem.name) !== -1} handleContextMenuClick={handleContextMenuClick} translucent={isDragging || (isOver && canDrop)}/>
      </div>)
  }
  ...
  return (<section ref={drop} className={classnames('FilesList no-select sans-serif border-box w-100 flex flex-column', className)}>
      {showLoadingAnimation
        ...
        : <Fragment>
          ...
          <WindowScroller>
            {({ height, isScrolling, onChildScroll, scrollTop }) => (<div className='flex-auto'>
                <AutoSizer disableHeight>
                  {({ width }) => (<List ref={listRef} autoHeight width={width} height={height} className='outline-0' aria-label={t('filesListLabel')} rowCount={rowCount} rowHeight={55} rowRenderer={rowRenderer} noRowsRenderer={emptyRowsRenderer} onRowsRendered={onRowsRendered} isScrolling={isScrolling} onScroll={onChildScroll} scrollTop={scrollTop}/>)}
                </AutoSizer>
              </div>)}
          </WindowScroller>
          {selectedFiles.length !== 0 && <SelectedActions className={'fixed bottom-0 right-0'} style={{
            zIndex: 20
          }} animateOnStart={selectedFiles.length === 1} unselect={() => toggleAll(false)} remove={() => onRemove(selectedFiles)} rename={() => onRename(selectedFiles)} share={() => onShare(selectedFiles)} setPinning={() => onSetPinning(selectedFiles)} download={() => onDownload(selectedFiles)} inspect={() => onInspect(selectedFiles[0].cid)} count={selectedFiles.length} isMfs={filesPathInfo.isMfs} downloadProgress={downloadProgress} size={selectedFiles.reduce((a, b) => a + (b.size || 0), 0)}/>}
        </Fragment>}
    </section>)
}
FilesList.propTypes = {
...
export default connect('selectPins', 'selectPinningServices', 'doFetchRemotePins', 'selectFilesIsFetching', 'selectFilesSorting', 'selectFilesPathInfo', 'selectShowLoadingAnimation', withTranslation('files')(FilesList))
