...
const DropableBreadcrumb = ({ index, link, immutable, onAddFiles, onMove, onClick, onContextMenuHandle, getPathInfo, checkIfPinned }) => {
    ...
    return (<span className='dib pv1 pr1' ref={drop}>
      <button ref={buttonRef} title={link.realName} className={classNames('BreadcrumbsButton relative', index !== 0 && 'navy', index === 0 && 'f7 pa1 br2 mr2', index === 0 && (immutable ? 'bg-charcoal-muted white' : 'bg-navy white'), immutable && (link.last || index === 0) && 'no-events', link.last && 'b', isOver && 'dragging')} onClick={() => onClick({ path: link.path })} onContextMenu={(ev) => index !== 0 && handleOnContextMenuHandle(ev)}>
        {link.name}
      </button>
    </span>);
};
const Breadcrumbs = ({ t, tReady, path, onClick, className, onContextMenuHandle, onAddFiles, onMove, doGetPathInfo, doCheckIfPinned, ...props }) => {
    ...
    return (<nav aria-label={t('breadcrumbs')} className={classNames('Breadcrumbs flex items-center sans-serif overflow-hidden sticky top-0', className)} {...props}>
      <div className='nowrap overflow-hidden relative flex flex-wrap' ref={anchors}>
        ...
        {bread.map((link, index) => (<div key={`${index}link`}>
            <DropableBreadcrumb index={index} link={link} immutable={isImmutable} onAddFiles={onAddFiles} onMove={onMove} onClick={onClick} onContextMenuHandle={onContextMenuHandle} getPathInfo={doGetPathInfo} checkIfPinned={doCheckIfPinned}/>
