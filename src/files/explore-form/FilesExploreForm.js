...
class FilesExploreForm extends React.Component {
    ...
    render() {
        const { t } = this.props;
        return (<div data-id='FilesExploreForm' className='sans-serif black-80 flex'>
        ...
        <div className='flex flex-row-reverse mb2'>
          <Button minWidth={0} disabled={!this.isValid} title={t('app:actions.inspect')} style={{ borderRadius: '0 3px 3px 0' }} onClick={this.onInspect} bg='bg-teal' className='ExploreFormButton button-reset pv1 ph2 ba f7 fw4 white overflow-hidden tc'>
            <StrokeIpld style={{ height: '2em' }} className='dib fill-current-color v-mid'/>
            <span className='ml2'>{t('app:actions.inspect')}</span>
          </Button>
          <Button minWidth={0} disabled={!this.isValid} style={{ borderRadius: '0' }} title={t('app:actions.browse')} onClick={this.onBrowse} className='ExploreFormButton button-reset pv1 ph2 ba f7 fw4 white bg-gray overflow-hidden tc'>
