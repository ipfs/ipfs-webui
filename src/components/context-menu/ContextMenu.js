...
const ContextMenu = ({ className, children, target, visible, arrowAlign, arrowMarginRight, onDismiss }) => {
    ...
    if (!visible || !target || !target.current)
        return null;
    const offsetLeft = getOffsetLeftByArrowAlignment(arrowAlign, { ...coordinates, arrowMarginRight });
    return (<Portal id="portal-dropdown">
      <div className={classNames('absolute', className)} style={{
            top: coordinates.top,
            left: coordinates.left
        }}>
        <DropdownMenu open={visible} className='sans-serif br2 charcoal' boxShadow='rgba(105, 196, 205, 0.5) 0px 1px 10px 0px' width='auto' arrowAlign={arrowAlign || 'center'} arrowMarginRight={`${arrowMarginRight}px`} top={coordinates.height} left={-coordinates.width - offsetLeft} translateX={-coordinates.width} onDismiss={onDismiss}>
