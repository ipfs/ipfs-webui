...
const pickTitleKey = (ipfsReady, ipfsConnected) => {
    if (ipfsReady && ipfsConnected)
        return 'app:status.connectedToIpfs';
    if (ipfsReady)
...
export const Connected = ({ t, className, size = 28, ipfsReady, ipfsConnected, doSetIsNodeInfoOpen }) => {
    ...
    return (<a title={title} className={`dib ${className}`} href='#/status' onClick={() => doSetIsNodeInfoOpen(true)}>
...
export default connect('selectIpfsReady', 'selectIpfsConnected', 'doSetIsNodeInfoOpen', withTranslation('notify')(Connected));
