        ...
        <ModalBody>
          ...
          <div className='pa2 pinningManagerServiceModalContainer'>
            ...
            <div className='relative'>
              <input id='cm-nickname' name='nickname' ref={register({ required: true })} type='text' className={classNames(inputClass, errors.nickname ? 'bg-red white' : 'charcoal')} placeholder={t('pinningServiceModal.nicknamePlaceholder')}/>
            ...
            <div className='relative'>
              <input id='cm-apiEndpoint' name='apiEndpoint' ref={register({ required: true, pattern: 'http(s){0,1}://.*' })} type='url' className={classNames(inputClass, errors.apiEndpoint ? 'bg-red white' : 'charcoal')} placeholder={t('pinningServiceModal.apiEndpointPlaceholder')}/>
            ...
            <label htmlFor="cm-secretApiKey">
              ...
              {service.icon && service.name && visitServiceUrl && (<a className='f7 link pv0 lh-copy dib' href={visitServiceUrl} rel='noopener noreferrer' target="_blank"> {t('pinningServiceModal.secretApiKeyHowToLink')}</a>)}
            </label>
            <div className='relative'>
              <input id='cm-secretApiKey' ref={register({ required: true })} name='secretApiKey' type='password' autoComplete='off' className={classNames(inputClass, errors.secretApiKey ? 'bg-red white' : 'charcoal')} placeholder="••••••••••••••••••••"/>
...
export default connect('doAddPinningService', withTranslation('settings')(PinningManagerServiceModal))
