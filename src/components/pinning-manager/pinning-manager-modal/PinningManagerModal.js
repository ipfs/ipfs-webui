    ...
    <Modal {...props} className={className} onCancel={onLeave} style={{ maxWidth: '34em' }}>
      <ModalBody>
        ...
        <div className="pa2 pinningManagerModalContainer">
          {remoteServiceTemplates.map(({ icon, name }) => (
            <button
              className="flex items-center pinningManagerModalItem pa1 hoverable-button"
              key={name}
              onClick={() => setSelectedService({ name, icon })}
            >
              <img
                className="mr3"
                src={icon}
        ...
        <p className="flex items-center justify-center">
          <Trans i18nKey="pinningModal.description" t={t}>
            ...
            <Button className="pv0" type="link" onClick={onCustomModalOpen}>
              Add a custom one.
            </Button>
          </Trans>
        </p>
      ...
      <Overlay show={!!selectedService} onLeave={onModalClose} hidden>
        <PinningServiceModal
          ...
          nickname={selectedServiceInfo.nickname}
          apiEndpoint={selectedServiceInfo.apiEndpoint}
          visitServiceUrl={selectedServiceInfo.visitServiceUrl}
          complianceReportUrl={selectedServiceInfo.complianceReportUrl}
          t={t}
        />
      </Overlay>
    </Modal>
