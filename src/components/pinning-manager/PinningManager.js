...
export const PinningManager = ({
  ...
  const localPinning = useMemo(
    () => ({
      name: t('localPinning'),
      type: 'LOCAL',
      totalSize: localPinsSize,
      numberOfPins: localNumberOfPins,
    }),
  ...
  const sortedServices = useMemo(
    () =>
      (pinningServices || []).sort(
        sortByProperty(
          sortSettings.sortBy,
          sortSettings.sortDirection === SortDirection.ASC ? 1 : -1
        )
      ),
    ...
    <Fragment>
            ...
            {({ width }) => (
              ...
              >
                ...
                <Column
                  label={t('pins')}
                  title={t('pins')}
                  dataKey="numberOfPins"
                  width={width * 0.2}
                  flexShrink={1}
                  cellRenderer={({ rowData }) => <NumberOfPinsCell rowData={rowData} t={t} />}
                  className="charcoal truncate f6 pl2"
                />
                <Column
                  label={t('autoUpload')}
                  title={t('autoUpload')}
                  ...
                  cellRenderer={({ rowData }) => (
                    <AutoUploadCell
                      autoUpload={rowData.autoUpload}
                      type={rowData.type}
                      name={rowData.name}
                      visitServiceUrl={rowData.visitServiceUrl}
                      doRemovePinningService={doRemovePinningService}
                      t={t}
        ...
        {arePinningServicesSupported && (
          <div className="flex justify-end w-100 mt2">
            <Button className="tc mt2" bg="bg-navy" onClick={onModalOpen}>
              <span>
                <span className="aqua">+</span> {t('actions.addService')}
              </span>
            </Button>
          </div>
        )}
      ...
      <Overlay show={isToggleModalOpen} onLeave={onToggleModalClose}>
        <AutoUploadModal
          ...
          t={t}
          name={isToggleModalOpen}
          service={sortedServices.find(s => s.name === isToggleModalOpen)}
        />
      </Overlay>
...
const NumberOfPinsCell = ({ rowData, t }) => {
  if (rowData.numberOfPins < 0) {
    return (
      <div className="red help" title={t('errors.failedToFetchTitle')}>
        {t('errors.failedToFetch')}
      </div>
    );
  }
  return (
    <div className={rowData.numberOfPins >= 0 ? '' : 'gray'}>
      {rowData.numberOfPins >= 0 ? rowData.numberOfPins : `${t('app:terms:loading')}...`}
    </div>
  );
};
const AutoUploadCell = ({
  autoUpload,
  visitServiceUrl,
  name,
  doRemovePinningService,
  ...
  <div className="flex justify-between items-center">
    <div className={!autoUpload ? 'gray' : ''}>
      {typeof autoUpload !== 'undefined' ? t('autoUploadPolicy.' + autoUpload) : '-'}
    </div>
    {type !== 'LOCAL' && (
      <OptionsCell
        doRemovePinningService={doRemovePinningService}
        ...
        visitServiceUrl={visitServiceUrl}
      />
    )}
  </div>
);
  ...
  return (
    <div>
      ...
      <ContextMenu
        ...
        target={buttonRef}
        onDismiss={() => setContextVisibility(false)}
        arrowAlign="right"
      >
        {showAutoUpload && (
          <ContextMenuItem className="pv2 ph1" onClick={() => onToggleModalOpen(name)}>
            <StrokeCloud width="28" className="fill-aqua" />{' '}
            <span className="ph1">
              {autoUpload
                ? t('pinningServices.removeAutoUpload')
                : t('pinningServices.addAutoUpload')}
            </span>
          </ContextMenuItem>
        )}
        {visitServiceUrl && (
          <a
            ...
            href={visitServiceUrl}
            target="_blank"
            rel="noopener noreferrer"
          >
            <ContextMenuItem className="pv2 ph1" onClick={() => setContextVisibility(false)}>
              <StrokeExternalLink width="28" className="fill-aqua" />{' '}
              <span className="ph1 charcoal">{t('visitService')}</span>
            </ContextMenuItem>
          </a>
        )}
        <ContextMenuItem className="pv2 ph1" onClick={handleRemove}>
          <StrokeCancel width="28" className="fill-aqua" />{' '}
          <span className="ph1">{t('remove')}</span>
        </ContextMenuItem>
      </ContextMenu>
    </div>
  );
