...
const ExampleRequest = ({ url, method = 'GET' }) => {
  return (
    <pre className='overflow-x-scroll pa3 mr3 f6 ba b--black-10 br2 spacegrotesk purple bg-near-purple'>
      <code className='white'>{method}</code> {url}
    </pre>
  )
}
...
const QueryParams = ({ url }) => {
  ...
  return (
    <dl className='pa3 mr3 f7 overflow-x-scroll spacegrotesk nowrap ba b--black-10 br2 bg-near-purple'>
      {entries.map(([key, value]) => (
        <div key={`QueryParams-${key}`}>
          <dt className='dib white'>{key}:</dt>
          <dd className='dib ml1 purple'>{value}</dd>
        </div>
      ))}
    </dl>
  )
    ...
    <section className='bb b--black-10'>
      <div className='flex items-center'>
        <Checkbox className='pv3 pl3 pr1 flex-none' onChange={onChange} checked={enabled} label={
          <span className='fw5 f6 white spacegrotesk'>{label}</span>
        } />
        <div className='truncate white fw4 f6 flex-auto'>&ndash; {summary}</div>
        <button className='bg-transparent focus-outline-none pa3 bn link flex-none text-gradient ' onClick={() => setOpen(!isOpen)} style={{ minWidth: 100 }}>
      ...
      {isOpen && (
        <div className='ph3 nt2 pb3'>
          ...
          {sourceLink && (
            <p>
              <a href={sourceLink} className='link purple' target='_blank' rel='noopener noreferrer'>View source</a>
            </p>
          )}
          {exampleRequest && (
            <div>
          ...
          <>
            <p>
              <Trans i18nKey='AnalyticsToggle.paragraph2' t={t}>
                Protocol Labs hosts a <a className='link purple' href='https://count.ly/'>Countly</a> instance to record anonymous usage data for this app.
              </Trans>
            </p>
              ...
              <ul>
                {analyticsActionsToRecord.map(name => (
                  <li key={name} className='mb1'>
                    <code className='f7 bg-near-purple pa1 br2'>{name}</code>
                  </li>
                ))}
              </ul>
            </AnalyticType>
