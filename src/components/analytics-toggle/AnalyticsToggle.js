    ...
    <section className="bg-white bb b--black-10">
      <div className="flex items-center">
        <Checkbox
          ...
          onChange={onChange}
          checked={enabled}
          label={<span className="fw5 f6">{label}</span>}
        />
        <div className="truncate fw4 f6 flex-auto charcoal-muted">&ndash; {summary}</div>
        <button
          className="focus-outline bg-transparent pa3 bn link blue flex-none"
      ...
      {isOpen && (
        <div className="ph3 nt2 pb3">
          ...
          {sourceLink && (
            <p>
              <a href={sourceLink} className="link blue" target="_blank" rel="noopener noreferrer">
                View source
              </a>
            </p>
          )}
  ...
  return (
    <React.Fragment>
      <Checkbox
        ...
        label={<span className="f5">{t('AnalyticsToggle.label')}</span>}
      />
        ...
        <Details summaryText={t('AnalyticsToggle.summary')} className="pt2" open={open}>
          <p>
            <Trans i18nKey="AnalyticsToggle.paragraph2" t={t}>
              Protocol Labs hosts a{' '}
              <a className="link blue" href="https://count.ly/">
                Countly
              </a>{' '}
              instance to record anonymous usage data for this app.
            </Trans>
          </p>
          ...
          >
            <p>
              <Trans i18nKey="AnalyticsToggle.events.details" t={t}>
                App specific actions. We record only that the action happened, how long it took from
                start to finish, and a count if the event involved multiple items.
              </Trans>
            </p>
          ...
          >
            <p>
              <Trans i18nKey="AnalyticsToggle.crashes.details" t={t}>
                Records JavaScript error messages and stack traces that occur while using the app,
                where possible. It is very helpful to know when the app is not working for you, but{' '}
                <b>error messages may include identifiable information</b> like CIDs or file paths,
                so only enable this if you are comfortable sharing that information with us.
              </Trans>
            </p>
          </AnalyticType>
        </Details>
