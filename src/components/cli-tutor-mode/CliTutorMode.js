      ...
      <ModalBody Icon={StrokeCode}>
        ...
        <p className="charcoal-muted w-90 center">
          {command && command === cliCommandList[cliCmdKeys.UPDATE_IPFS_CONFIG]()
            ? t('settings:cliModal.extraNotesJsonConfig')
            : ''}
          {command && command.startsWith(cliCmdPrefixes.PIN_OBJECT)
            ? t('files:cliModal.extraNotesPinning')
            : ''}
        </p>
        <div>
      ...
      <Fragment>
        {showIcon ? (
          <StrokeCode
            ...
            className="dib fill-link pointer mh2"
            style={{ height: 44 }}
          />
        ) : (
          <div />
