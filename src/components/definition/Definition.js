...
export const Definition = ({ term, desc, advanced, termWidth }) => (<div className='dt dt--fixed pt1 mw9 lh-copy'>
...
export const Term = ({ children, width = 100 }) => {
    return (<dt className='db ma0 pb1 pb0-ns dtc-ns silver tracked ttu f7' style={{ width }}>
      {children}
    </dt>);
};
export const Description = ({ children, advanced }) => {
    return (<dd className={`db dtc-ns ma0 charcoal monospace ${advanced ? 'word-wrap pa2 f7 bg-white-80' : 'truncate f7 f6-ns'}`}>
      {children}
    </dd>);
};
