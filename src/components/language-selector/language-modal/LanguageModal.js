    ...
    return (<Modal {...props} className={className} onCancel={onLeave} style={{ maxWidth: '40em' }}>
      <ModalBody Icon={SpeakerIcon}>
        ...
        <div className='pa2 flex flex-wrap'>
          {localesList.map((lang) => <button key={`lang-${lang.locale}`} className='pa2 w-33 flex nowrap bg-transparent bn outline-0 blue justify-center' onClick={() => handleClick(lang.locale)}>
              {lang.nativeName}
            </button>)}
        </div>
