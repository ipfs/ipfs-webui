...
const PublicGatewayForm = ({ t, doUpdatePublicGateway, publicGateway }) => {
  ...
  return (<form onSubmit={onSubmit}>
      <input id='public-gateway' aria-label={t('terms.publicGateway')} placeholder={t('publicGatewayForm.placeholder')} type='text' className={`w-100 lh-copy monospace f5 pl1 pv1 mb2 charcoal input-reset ba b--black-20 br1 ${showFailState ? 'focus-outline-red b--red-muted' : 'focus-outline-green b--green-muted'}`} onChange={onChange} onKeyPress={onKeyPress} value={value}/>
      <div className='tr'>
        <Button minWidth={100} height={40} bg='bg-charcoal' className='tc' disabled={value === DEFAULT_GATEWAY} onClick={onReset}>
          {t('app:actions.reset')}
        </Button>
        <Button minWidth={100} height={40} className='mt2 mt0-l ml2-l tc' disabled={!isValidGatewayUrl || value === publicGateway}>
          {t('actions.submit')}
        </Button>
      </div>
    </form>)
}
export default connect('doUpdatePublicGateway', 'selectPublicGateway', withTranslation('app')(PublicGatewayForm))
