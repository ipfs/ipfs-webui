...
const RenameKeyModal = ({ t, tReady, name, onCancel, onSubmit, className, ...props }) => {
  return (<TextInputModal onSubmit={(p) => onSubmit(p.trim())} onChange={(p) => p.trimStart()} onCancel={onCancel} className={className} title={t('renameKeyModal.title')} description={<div className='charcoal w-90 center tl mb3'>
        ...
        </div>} Icon={Icon} defaultValue={name} mustBeDifferent={true} submitText={t('app:actions.rename')} {...props}/>)
}
