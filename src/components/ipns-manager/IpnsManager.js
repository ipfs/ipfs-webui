  ...
  <div className="flex justify-end">
    {name !== 'self' && (
      <OptionsCell
        name={name}
        t={t}
        showRenameKeyModal={showRenameKeyModal}
        showRemoveKeyModal={showRemoveKeyModal}
      />
    )}
  </div>
);
    ...
    <div>
      ...
      >
        <ContextMenuItem className="pv2 ph1" onClick={handle(showRenameKeyModal)}>
          <StrokePencil width="28" className="fill-aqua" />{' '}
          <span className="ph1">{t('app:actions.rename')}</span>
        </ContextMenuItem>
        <ContextMenuItem className="pv2 ph1" onClick={handle(showRemoveKeyModal)}>
          <StrokeCancel width="28" className="fill-aqua" />{' '}
          <span className="ph1">{t('app:actions.remove')}</span>
        </ContextMenuItem>
      </ContextMenu>
    </div>
  );
      ...
      <div className="mv4 pinningManager">
              ...
              >
                ...
                <Column
                  ...
                  cellRenderer={({ rowData }) =>
                    rowData.published ? (
                      <a
                        href={`${availableGateway}/ipns/${rowData.id}`}
                        target="_blank"
        ...
        <div className="flex justify-end w-100 mt2">
          <Button className="tc mt2" bg="bg-navy" onClick={showGenerateKeyModal}>
            <span>
              <span className="aqua">+</span> {t('actions.generateKey')}
            </span>
          </Button>
        </div>
      </div>
