...
bundle.selectConfigObject = createSelector('selectConfig', (configStr) => JSON.parse(configStr));
bundle.selectApiUrl = createSelector('selectConfigObject', 'selectPublicGateway', (config, publicGateway) => getURLFromAddress('API', config) || publicGateway);
bundle.selectGatewayUrl = createSelector('selectConfigObject', 'selectPublicGateway', (config, publicGateway) => getURLFromAddress('Gateway', config) || publicGateway);
bundle.selectAvailableGatewayUrl = createSelector('selectAvailableGateway', 'selectGatewayUrl', (availableGateway, gatewayUrl) => availableGateway || gatewayUrl);
bundle.selectBootstrapPeers = createSelector('selectConfigObject', (config) => config && config.Bootstrap);
...
bundle.selectIsConfigBlocked = createSelector('selectConfigRaw', ({ errorType }) => errorType === 'Access to config.get API is globally blocked for window.ipfs');
// Fetch the config if we don't have it or it's more than `staleAfter` ms old
bundle.reactConfigFetch = createSelector('selectConfigShouldUpdate', 'selectIpfsReady', (shouldUpdate, ipfsReady) => {
...
function getURLFromAddress(name, config) {
    ...
    try {
        ...
        if (new URL(url).port === 0)
            throw Error('port set to 0, not deterministic');
        return url;
    }
    catch (error) {
...
const checkIfGatewayUrlIsAccessible = memoize(async (url) => {
    try {
        const { status } = await fetch(`${url}/ipfs/bafkqae2xmvwgg33nmuqhi3zajfiemuzahiwss`);
        return status === 200;
    }
    catch (e) {
