...
it('should sync added peers', async () => {
    const store = composeBundlesRaw(createReactorBundle(), mockRoutesBundle, createMockIpfsBundle(createMockIpfs()), mockPeersBundle, createPeerBandwidthBundle())();
...
it('should get bandwidth for added peers', async () => {
    ...
    const peers = await fakePeers(totalPeers);
    const store = composeBundlesRaw(createReactorBundle(), mockRoutesBundle, 
    // This IPFS takes at minimum 20ms to respond to a function call
    createMockIpfsBundle(createMockIpfs({ minLatency: 20, maxLatency: 30 })), mockPeersBundle, 
...
it('should periodically update bandwidth for peers', async () => {
    ...
    const peers = await fakePeers(totalPeers);
    const store = composeBundlesRaw(createReactorBundle(), mockRoutesBundle, createMockIpfsBundle(createMockIpfs({ minLatency: 0, maxLatency: 1 })), mockPeersBundle, 
    // Up the concurrency value for the bundle so all the bandwidth updates
    // are fired off at the same time
...
it('should update peer bandwidth according to concurrency option', async () => {
    ...
    const store = composeBundlesRaw(createReactorBundle(), mockRoutesBundle, createMockIpfsBundle(createMockIpfs({ minLatency: 100, maxLatency: 150 })), mockPeersBundle, createPeerBandwidthBundle({ peerUpdateConcurrency }))({
        peers: { data: peers }
    });
    await sleep(50);
