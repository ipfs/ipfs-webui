...
const createMockIpfs = opts => {
  ...
  return {
    stats: {
      bw: async function* () {
        const bw = await new Promise(resolve =>
          setTimeout(() => resolve(fakeBandwidth()), randomInt(opts.minLatency, opts.maxLatency))
        );
        yield bw;
      },
    },
...
it('should sync added and removed peers', async () => {
  ...
  const nextPeers = peers.slice(0, totalRemovedPeers).concat(await fakePeers(totalAddedPeers));
