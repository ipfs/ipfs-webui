...
const notIn =
  (...arrays) =>
  p =>
    arrays.every(array => !array.some(a => a === p));
...
const parseService = async (service, remoteServiceTemplates, ipfs) => {
  const template = remoteServiceTemplates.find(
    t => service.endpoint.toString() === t.apiEndpoint.toString()
  );
...
const mfsPolicyEnableFlag = memoize(
  async (serviceName, ipfs) => {
    try {
      ...
      if (e.message?.includes('key has no attribute')) {
        try {
          // retry with notation from https://github.com/ipfs/kubo/pull/8096
          return await ipfs.config.get(
            `Pinning.RemoteServices["${serviceName}"].Policies.MFS.Enable`
          );
        } catch (_) {}
      }
...
const intervalFetchPins = store => {
  setInterval(() => {
    const pins = [...store.selectPendingPins(), ...store.selectFailedPins()].map(serviceCid => ({
      cid: serviceCid.split(':')[1],
    }));
...
const pinningBundle = {
  ...
  reducer: (state, action) => {
    if (action.type === 'UPDATE_REMOTE_PINS') {
      ...
      const remotePins = getRerenderAwareArray(
        state.remotePins,
        uniq([...state.remotePins, ...adds].filter(notIn(removals, pending, failed)))
      );
      const notRemotePins = getRerenderAwareArray(
        state.notRemotePins,
        uniq([...state.notRemotePins, ...removals].filter(notIn(adds, pending, failed)))
      );
      const pendingPins = getRerenderAwareArray(
        state.pendingPins,
        uniq([...state.pendingPins, ...pending].filter(notIn(adds, removals, failed)))
      );
      const failedPins = getRerenderAwareArray(
        state.failedPins,
        uniq([...state.failedPins, ...failed].filter(notIn(adds, removals, pending)))
      );
      const completedPins = getRerenderAwareArray(
        state.completedPins,
        uniq([...state.completedPins, ...adds].filter(p => state.pendingPins.some(a => a === p)))
      );
  ...
  doFetchRemotePins:
    (files, skipCache = false) =>
    async ({ dispatch, store, getIpfs }) => {
      ...
      await Promise.allSettled(
        pinningServices.map(async service => {
          try {
            ...
            for (const cidChunk of cids) {
              ...
              try {
                const pins = remotePinLs(ipfs, {
                  service: service.name,
                  cid: cidsToCheck.map(cid => CID.asCID(cid) ?? CID.parse(cid)),
                ...
                console.error(
                  `Error: pin.remote.ls service=${service.name} cid=${cidsToCheck}: ${e.toString()}`
                );
              }
  ...
  doSelectRemotePinsForFile:
    ...
    ({ store }) => {
      ...
      const remoteAndPendingPins = uniq([
        ...store.selectPendingPins(),
        ...store.selectRemotePins(),
      ]);
      const remoteAndPendingPinsForFile = remoteAndPendingPins.filter(
        pin => cacheId2Cid(pin) === file.cid.toString()
      );
      const servicesBeingUsed = remoteAndPendingPinsForFile
        .map(pin => cacheId2ServiceName(pin))
        .filter(name => pinningServicesNames.includes(name));
  ...
  doFetchPinningServices:
    () =>
    async ({ getIpfs, store, dispatch }) => {
      const ipfs = getIpfs();
      if (!ipfs || store?.ipfs?.ipfs?.ready || !ipfs.pin.remote) return null;

      const isPinRemotePresent = (await ipfs.commands()).Subcommands.find(
        c => c.Name === 'pin'
      ).Subcommands.some(c => c.Name === 'remote');
      ...
      const remoteServices = await Promise.all(
        offlineListOfServices.map(service => parseService(service, remoteServiceTemplates, ipfs))
      );
  ...
  doFetchPinningServicesStats:
    ...
    async ({ getIpfs, store, dispatch }) => {
      ...
      const remoteServices = await Promise.all(
        servicesWithStats.map(service => parseService(service, remoteServiceTemplates, ipfs))
      );
  ...
  selectPinningServicesDefaults: () =>
    pinningServiceTemplates.reduce(
      (prev, curr) => ({
        ...prev,
        [curr.name]: {
          ...curr,
  ...
  doSetPinning:
    (file, services = [], wasLocallyPinned, previousRemotePins = []) =>
    async ({ getIpfs, store, dispatch }) => {
      const ipfs = getIpfs();
      const { cid, name } = file;
  ...
  doAddPinningService:
    ({ apiEndpoint, nickname, secretApiKey }) =>
    async ({ getIpfs }) => {
      const ipfs = getIpfs();
