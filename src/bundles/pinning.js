...
const pinningBundle = {
    ...
    doFetchRemotePins: (files, skipCache = false) => async ({ dispatch, store, getIpfs }) => {
        ...
        if (!pinningServices?.length)
            return;
        const ipfs = getIpfs();
        if (!ipfs || store?.ipfs?.ipfs?.ready || !ipfs.pin.remote)
            return;
        ...
        await Promise.allSettled(pinningServices.map(async (service) => {
            try {
                ...
                const skipCids = skipCache
                    ? new Set()
                    : new Set([...remotePins, ...notRemotePins]
                ...
                for (const cidChunk of cids) {
                    ...
                    if (!cidsToCheck.length)
                        continue; // skip if no new cids to check
