...
const bundle = function (opts) {
  ...
  return {
    ...
    reducer(state = { peers: [], updatingPeerIds: [], now: Date.now() }, action) {
      ...
      return Date.now() - state.now > opts.tickResolution ? { ...state, now: Date.now() } : state;
    },
    ...
    selectNextPeerIdForUpdate: createSelector(
      ...
      (now, peers, updatingPeerIds) => {
        ...
        const peerUpdateInterval = p =>
          p.active ? opts.peerUpdateInterval : opts.inactivePeerUpdateInterval;
    ...
    doUpdatePeerBandwidth:
      peerId =>
      async ({ dispatch, getState, getIpfs }) => {
