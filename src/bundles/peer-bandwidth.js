...
const bundle = function (opts) {
    ...
    return {
        ...
        selectNextPeerIdForUpdate: createSelector('selectPeerBandwidthNow', 'selectPeerBandwidthPeers', 'selectPeerBandwidthUpdatingPeerIds', (now, peers, updatingPeerIds) => {
        ...
        reactUpdatePeerBandwidth: createSelector('selectRouteInfo', 'selectIpfsReady', 'selectNextPeerIdForUpdate', (routeInfo, ipfsReady, peerId) => {
            if (routeInfo.url === '/' && ipfsReady && peerId) {
                return { actionCreator: 'doUpdatePeerBandwidth', args: [peerId] };
            }
        }),
        reactSyncPeers: createSelector('selectRouteInfo', 'selectPeers', 'selectPeerBandwidthPeers', (routeInfo, peers, bwPeers) => {
            if (routeInfo.url !== '/')
                return;
