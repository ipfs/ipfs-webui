...
const IMG_ARRAY = [
  { id: 'IMG_HASH_1PX', name: '1x1.png', hash: IMG_HASH_1PX },
  {
    id: 'IMG_HASH_1PXID',
    name: '1x1.png',
    hash: 'bafkqax4jkbheodikdifaaaaabveuqrcsaaaaaaiaaaaacaidaaaaajo3k3faaaaaanieyvcfaaaabj32hxnaaaaaaf2fetstabaonwdgaaaaacsjiravicgxmnqaaaaaaiaadyrbxqzqaaaaabeuktsevzbgbaq',
  },
  {
    id: 'IMG_HASH_FAVICON',
    name: 'favicon.ico',
    hash: 'bafkreihc7efnl2prri6j6krcopelxms3xsh7undpsjqbfsasm7ikiyha4i',
  },
];
...
export const checkViaImgSrc = gatewayUrl => {
   ...
   */
  return Promise.any(
    IMG_ARRAY.map(element => {
      const imgUrl = new URL(
        `${url.protocol}//${url.hostname}/ipfs/${element.hash}?now=${Date.now()}&filename=${element.name}#x-ipfs-companion-no-redirect`
      );
  ...
  return new Promise((resolve, reject) => {
    ...
    let timer = setTimeout(() => {
      if (timeout())
        reject(
          new Error(
            `Image load timed out after ${imgCheckTimeout / 1000} seconds for URL: ${imgUrl}`
          )
        );
    }, imgCheckTimeout);
    const img = new Image();
...
export async function checkSubdomainGateway(gatewayUrl) {
  ...
  try {
    ...
    imgSubdomainUrl = new URL(
      `${gwUrl.protocol}//${IMG_HASH_1PX}.ipfs.${gwUrl.hostname}/?now=${Date.now()}&filename=1x1.png#x-ipfs-companion-no-redirect`
    );
    imgRedirectedPathUrl = new URL(
      `${gwUrl.protocol}//${gwUrl.hostname}/ipfs/${IMG_HASH_1PX}?now=${Date.now()}&filename=1x1.png#x-ipfs-companion-no-redirect`
    );
  } catch (err) {
  ...
  return await checkViaImgUrl(imgSubdomainUrl)
    ...
    .then(() => {
      console.log(
        `Gateway at '${gatewayUrl}' is functioning correctly (verified image loading and redirection)`
      );
      return true;
    })
    .catch(err => {
...
const bundle = {
  ...
  doSetAvailableGateway:
    url =>
    ({ dispatch }) =>
      dispatch({ type: 'SET_AVAILABLE_GATEWAY', payload: url }),
