...
const mockPeersBundle = {
  ...
  reducer(state = { data: [] }, action) {
    return action.type === 'UPDATE_MOCK_PEERS' ? { ...state, data: action.payload } : state;
  },
...
async function getPeerLocationsFromStore({ store, failMs = 5000 }) {
  return new Promise((resolve, reject) => {
    const timeout = setTimeout(
      () => reject(new Error(`Timed out waiting ${failMs}ms for peerLocations from store`)),
      failMs
    );
...
describe('reactPeerLocationsFetch', () => {
  it.skip('should declare its dependencies', async () => {
    ...
    expect(createSelector).toHaveBeenNthCalledWith(
      1,
      'selectRouteInfo',
  ...
  it('should do nothing when the route is not peers or its not time to update', () => {
    const { reactPeerLocationsFetch } = createPeersLocationBundle();
    expect(
      callSelectorMethod(reactPeerLocationsFetch, { url: '/another-url' }, true, true)
    ).toBeFalsy();
    expect(callSelectorMethod(reactPeerLocationsFetch, { url: '/peers' }, false, true)).toBeFalsy();
    expect(
      callSelectorMethod(reactPeerLocationsFetch, { url: '/peers' }, false, false)
    ).toBeFalsy();
...
describe('selectPeerLocationsForSwarm', () => {
  ...
  it('should map the peers with the location information', async () => {
    ...
    const result = await callSelectorMethod(
  ...
  it('should also handle the public ip', async () => {
    ...
    const identity = {
      addresses: [{ address: '127.0.0.1' }, { address: '2001:0db8:85a3:0000:0000:8a2e:0370:7334' }],
    };
...
describe('PeerLocationResolver', () => {
    ...
    it('should find the location of given peers', async () => {
      ...
      const store = await createStore({
        selectors: {
          ...
          selectIdentity: () => ({
            addresses: ['/p2p/1aaa1', '/ip4/4.4.4.4/udp/1234'],
          }),
        },
      });
  ...
  describe('optimizedPeerSet', () => {
    it('should return sets of 10, 100, 200 peers and more according to the number of calls', async () => {
      ...
      const expect100 = new Array(90).fill().reduce(
        (prev, _, index) => ({
          ...prev,
      ...
      const expect200 = new Array(100).fill().reduce(
        (prev, _, index) => ({
          ...prev,
      ...
      const expectMore = new Array(800).fill().reduce(
        (prev, _, index) => ({
          ...prev,
