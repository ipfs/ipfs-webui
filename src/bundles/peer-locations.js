...
function createPeersLocations(opts) {
  ...
  const bundle = createAsyncResourceBundle({
    ...
    getPromise: ({ store }) =>
      peerLocResolver.findLocations(store.selectAvailableGatewayUrl(), store.selectPeers()),
    ...
    (peers, locations = {}, bootstrapPeers, identity) =>
      ...
      Promise.all(
        peers.map(async peer => {
          ...
          const coordinates = locationObj && [locationObj.longitude, locationObj.latitude];
          ...
          const protocols = (
            Array.isArray(peer.streams)
              ? Array.from(
                  new Set(
                    peer.streams.map(s => s.Protocol).map(p => (!p?.trim() ? '[unnamed]' : p)) // mark weird 'empty' protocols
                  )
                ).sort()
              : []
          ).join(', ');
  ...
  bundle.selectPeersCoordinates = createSelector('selectPeerLocationsForSwarm', async peers => {
    if (!peers) return [];
    ...
    return fetchedPeers.reduce((previous, { peerId, coordinates }) => {
      ...
      const previousCoordinates = previous.map(prev => {
        ...
        const isCloseInXAxis =
          x - COORDINATES_RADIUS <= currentX && x + COORDINATES_RADIUS >= currentX;
        const isCloseInYAxis =
          y - COORDINATES_RADIUS <= currentY && y + COORDINATES_RADIUS >= currentY;
...
const getPublicIP = memoize(identity => {
  ...
  for (const maddr of identity.addresses) {
    try {
      ...
      if (
        (ip.isV4Format(addr.address) || ip.isV6Format(addr.address)) &&
        !ip.isPrivate(addr.address)
      ) {
        return addr.address;
      }
    } catch (e) {
...
const isPrivateAndNearby = async (maddr, identity) => {
  ...
  if (publicIP) {
    if (ip.isV4Format(addr.address)) {
      ...
      isNearby =
        ip.cidrSubnet(`${publicIP}/48`).contains(addr.address) &&
...
class PeerLocationResolver {
  ...
  async findLocations(gatewayUrls, peers) {
    ...
    gatewayUrls = (Array.isArray(gatewayUrls) ? gatewayUrls : [gatewayUrls]).map(url =>
      url.replace(/localhost:(\d+)/, '127.0.0.1:$1')
    );
