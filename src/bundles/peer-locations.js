...
function createPeersLocations(opts) {
    ...
    bundle.reactPeerLocationsFetch = createSelector('selectRouteInfo', 'selectPeerLocationsShouldUpdate', 'selectIpfsConnected', (routeInfo, shouldUpdate, ipfsConnected) => {
        if (routeInfo.url === '/peers' && shouldUpdate && ipfsConnected) {
            return { actionCreator: 'doFetchPeerLocations' };
        }
    });
    bundle.selectPeerLocationsForSwarm = createSelector('selectPeers', 'selectPeerLocations', 'selectBootstrapPeers', 'selectIdentity', // ipfs.id info for local node, used for detecting local peers
    ...
    bundle.selectPeersCoordinates = createSelector('selectPeerLocationsForSwarm', peers => {
        if (!peers)
            return [];
        return peers.reduce((previous, { peerId, coordinates }) => {
            ...
            const previousCoordinates = previous.map(prev => {
                if (!prev || hasFoundACloseCoordinate)
                    return prev;
