...
export const cliCommandList = {
   ...
   * @param {string} path
   */
  [cliCmdKeys.ADD_FILE]: path =>
    `ipfs files cp /ipfs/$(ipfs add -Q <local-file>) "${path}/<dest-name>"`,
  /**
   * @param {string} path
   */
  [cliCmdKeys.ADD_DIRECTORY]: path =>
    `ipfs files cp /ipfs/$(ipfs add -r -Q <local-folder>) "${path}/<dest-name>"`,
  /**
   * @param {string} path
   */
   ...
   * @param {string} path
   */
  [cliCmdKeys.FROM_CAR]: path =>
    `ipfs dag import file.car && ipfs files cp /ipfs/<imported-cid> "${path}/<dest-name>"`,
  [cliCmdKeys.ADD_NEW_PEER]: () => 'ipfs swarm connect <peer-multiaddr>',
  /**
   * @param {string} ipfsPath
   * @param {string} name
   */
  [cliCmdKeys.PUBLISH_WITH_IPNS]: (ipfsPath, name) =>
    `ipfs name publish ${ipfsPath} --key="${name}"`,
  /**
   * @param {string} cid
   */
