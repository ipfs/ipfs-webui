...
const actions = () => ({
   ...
   */
  doPinsFetch: () =>
    perform(ACTIONS.PIN_LIST, async ipfs => {
   ...
   */
  doFetch: ({ path, realPath, isMfs, isRoot }) =>
    perform(ACTIONS.FETCH, async (ipfs, { store }) => {
      if (isRoot && !isMfs) {
   ...
   * @param {string} root
   */
  doFilesWrite: (source, root) =>
    spawn(ACTIONS.WRITE, async function* (ipfs, { store }) {
      const files = source
        // Skip ignored files
   ...
   */
  doFilesDelete: ({ files, removeLocally, removeRemotely, remoteServices }) =>
    perform(ACTIONS.DELETE, async (ipfs, { store }) => {
      ensureMFS(store);
      ...
      try {
        ...
        await Promise.all(
          files.map(async file =>
            ipfs.files.rm(realMfsPath(file.path), {
              recursive: true,
            })
          )
        );
        ...
        if (removeRemotely) {
          ...
          files.forEach(file =>
            remoteServices.map(async service =>
              tryAsync(() => ipfs.pin.remote.rm({ cid: [file.cid], service }))
            )
          );
        }
        ...
        if (removeLocally) {
          ...
          await Promise.all(
            files.map(async file => file.pinned && tryAsync(() => ipfs.pin.rm(file.cid)))
          );
        }
   ...
   * @param {string} name
   */
  doFilesAddPath: (root, src, name = '') =>
    perform(ACTIONS.ADD_BY_PATH, async (ipfs, { store }) => {
      ensureMFS(store);
   ...
   * @param {string} name
   */
  doAddCarFile: (root, carFile, name = '') =>
    perform(ACTIONS.ADD_CAR_FILE, async (/** @type {IPFSService} */ ipfs, { store }) => {
      ensureMFS(store);
      ...
      try {
        ...
        if (dstExists) {
          throw new Error(
            `The name "${name}" already exists in the current directory. Try importing with a different name.`
          );
        }
   ...
   */
  doFilesBulkCidImport: (source, root) =>
    perform(ACTIONS.BULK_CID_IMPORT, async function (ipfs, { store }) {
      ensureMFS(store);
   ...
   */
  doFilesDownloadLink: files =>
    perform(ACTIONS.DOWNLOAD_LINK, async (ipfs, { store }) => {
   ...
   * @param {FileStat[]} files
   */
  doFilesShareLink: files =>
    perform(ACTIONS.SHARE_LINK, async (ipfs, { store }) => {
   ...
   */
  doFilesMove: (src, dst) =>
    perform(ACTIONS.MOVE, async (ipfs, { store }) => {
      ensureMFS(store);
   ...
   */
  doFilesCopy: (src, dst) =>
    perform(ACTIONS.COPY, async (ipfs, { store }) => {
      ensureMFS(store);
   ...
   */
  doFilesMakeDir: path =>
    perform(ACTIONS.MAKE_DIR, async (ipfs, { store }) => {
      ensureMFS(store);
   ...
   */
  doFilesPin: cid =>
    perform(ACTIONS.PIN_ADD, async (ipfs, { store }) => {
      try {
   ...
   */
  doFilesUnpin: cid =>
    perform(ACTIONS.PIN_REMOVE, async (ipfs, { store }) => {
      try {
   ...
   */
  doFilesSizeGet: () =>
    perform(ACTIONS.SIZE_GET, async ipfs => {
...
const dirStats = async (ipfs, cid, { path, isRoot, sorting }) => {
  ...
  const res = entries.length === 1 && entries[0].cid.toString() === cid.toString() ? [] : entries;
  const files = [];
