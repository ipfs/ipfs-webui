...
const actions = () => ({
    ...
    doFilesDelete: ({ files, removeLocally, removeRemotely, remoteServices }) => perform(ACTIONS.DELETE, async (ipfs, { store }) => {
        ...
        try {
            ...
            await Promise.all(files.map(async (file) => ipfs.files.rm(realMfsPath(file.path), {
                recursive: true
            })));
            // Pin cleanup only if MFS removal was successful
            if (removeRemotely) {
                // remote unpin can be slow, so we do this async in best-effort fashion
                files.forEach(file => remoteServices.map(async (service) => tryAsync(() => ipfs.pin.remote.rm({ cid: [file.cid], service }))));
            }
            if (removeLocally) {
                // removal of local pin can fail if same CID is present twice,
                // this is done in best-effort as well
                await Promise.all(files.map(async (file) => file.pinned && tryAsync(() => ipfs.pin.rm(file.cid))));
            }
