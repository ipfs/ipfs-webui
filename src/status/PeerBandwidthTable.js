...
export class PeerBandwidthTable extends Component {
    ...
    getSorter({ field, direction }) {
        return (a, b) => {
            if (a.bw[field].gt(b.bw[field]))
                return direction;
            if (a.bw[field].lt(b.bw[field]))
                return -direction;
            return 0;
        };
    }
    ...
    render() {
        ...
        <table className='collapse'>
          <tbody>
            ...
            {visiblePeers.map((p, i) => (<tr key={p.id} className={i % 2 ? 'bg-snow-muted' : ''}>
...
function SortableTableHeader({ field, label, sort, onClick }) {
    return (<th className='pv2 ph3 pointer underline-hover nowrap' onClick={onClick} data-field={field}>
...
function LocationFlag({ location }) {
    if (!location)
        return '🏳️‍🌈';
    return (<span title={location.country_name}>
      <CountryFlag code={location.country_code} svg={isWindows}/>
    </span>);
}
export default connect('selectPeerBandwidthPeers', 'selectPeerLocations', withTranslation('status')(PeerBandwidthTable));
