    ...
    return sortedPeers.length === 0 ? (
      ...
      <Box className={className}>
        ...
        <table className="collapse">
          <tbody>
            <tr className="tl">
              <th className="f6 pv2 pr3 pl0" colSpan="2">
                <span className="v-mid">{t('app:terms.peer')}</span>
              </th>
              <SortableTableHeader
                field="rateIn"
                label={t('app:rateIn')}
            ...
            {visiblePeers.map((p, i) => (
              <tr key={p.id} className={i % 2 ? 'bg-snow-muted' : ''}>
                <td className="f6 pv2 nowrap">
                  <LocationFlag location={peerLocations[p.id]} />
                </td>
                <td className="f6 pv2 ph3 w-100 monospace">{p.id}</td>
                <td className="f6 pv2 ph3 nowrap">
                  {humansize(parseInt(p.bw.rateIn.toFixed(0), 10))}/s
                </td>
                <td className="f6 pv2 ph3 nowrap">
                  {humansize(parseInt(p.bw.rateOut.toFixed(0), 10))}/s
                </td>
                <td className="f6 pv2 ph3 nowrap">
                  {humansize(parseInt(p.bw.totalIn.toFixed(0), 10))}
                </td>
                <td className="f6 pv2 ph3 nowrap">
                  {humansize(parseInt(p.bw.totalOut.toFixed(0), 10))}
                </td>
              </tr>
            ))}
          </tbody>
        ...
        {!showAll && hiddenPeers.length ? (
          <button
            className="sans-serif f5 ma0 buttonv3 ph2 tc pointer underline-hover navy-muted"
            onClick={this.onShowAllClick}
          >
            {t('countMore', { count: hiddenPeers.length })}
          </button>
        ) : null}
      </Box>
    );
...
function SortArrow({ field, sortField, direction }) {
  ...
  return (
    <img
      src={src}
      className="v-mid"
      alt={'Sorted ' + (direction === 1 ? 'ascending' : 'descending')}
    />
  );
}
