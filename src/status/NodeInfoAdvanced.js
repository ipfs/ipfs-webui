...
const NodeInfoAdvanced = ({
  ...
  if (identity) {
    ...
    addresses = [...new Set(identity.addresses)]
      .sort()
      .map(addr => <Address key={addr} value={addr} />);
  }
  ...
  const asAPIString = value => {
    ...
    return typeof value !== 'string' ? t('customApiConfig') : value;
  };
          ...
          <Definition
            ...
            desc={
              <div id="http-api-address" className="flex items-center">
                ...
                <a className="ml2 link blue sans-serif fw6" href="#/settings">
                  {t('app:actions.edit')}
                </a>
              </div>
            }
