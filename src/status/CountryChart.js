...
const CountryChart = ({ t, peerLocations, className }) => {
  ...
  if (countryRanking.length < 5) {
    data = countryRanking.map(code => Math.round((countsByCountry[code] / totalCountries) * 100));
  } else {
    data = countryRanking
      .slice(0, 3)
  ...
  return (
    <div>
      <Title>
        <a className="link aqua" href="#/peers">
          {t('distributionOfPeers')}
        </a>
      </Title>
...
export default connect('selectPeerLocations', withTranslation('status')(CountryChart));
